...
The server exposes 11 tools covering the full POP API surface: Italian e-invoicing (FatturaPA/SdI), Peppol (EU cross-border), PDF generation, document validation, archival, and status tracking.
...
└── tools/
    ...
    └── advanced.ts       pop_verify_sdi_document, pop_send_sdi_document, pop_preserve_document,
                          pop_verify_fiscal_id, pop_verify_company
```
...
| `pop_send_sdi_document` | POST `/sdi-via-pop/document-send` | Growth+ |
| `pop_preserve_document` | POST `/sdi-via-pop/document-preserve` | Growth+ |
| `pop_verify_fiscal_id` | POST `/sdi-via-pop/verify-fiscal-id` | Any |
| `pop_verify_company` | POST `/sdi-via-pop/verify-company` | Any |
...
- [x] All 11 tools implemented and registered
